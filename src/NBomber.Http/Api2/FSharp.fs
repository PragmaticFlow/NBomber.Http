namespace NBomber.Http.Api2.FSharp
...
module Http =
    ...
    let withHeaders (headers: (string*string) list) (req: HttpRequest) =
        { req with Headers = headers |> Array.ofList }
...
type HttpStep =    
    ...
    [<CompiledName "Create">] // with this attribute it could be the same for F# and C#
    ...
    [<CompiledName "WithRequest">]
    static member withRequest name req =
        HttpStep.create(name, fun _ -> Task.FromResult req)